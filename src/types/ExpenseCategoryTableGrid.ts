...
import {
  CategoryBodyGridCell,
  CategoryBodyGridCellCheckbox,
} from './ExpenseCategoryTableGridTypes';

const sortSummariesByCategory = (
  summaries: Array<ExpenseCategorySummary>
): Array<ExpenseCategorySummary> => {
  return [...summaries].sort(
    (a: ExpenseCategorySummary, b: ExpenseCategorySummary): number => {
...
const sortSummariesByMonthColumn = (
  summaries: Array<ExpenseCategorySummary>,
  columnIndex: number
): Array<ExpenseCategorySummary> => {
  return [...summaries].sort(
...
const getRowTotalCell = (
  summary: ExpenseCategorySummary
): CategoryBodyGridCell => {
  return new CategoryBodyGridCell(
    summary.getCategory(),
    new MonthTotal(MonthTotal.FAKE_MONTH, summary.getTotal()),
    null
  );
};

const getRowAverageCell = (
  summary: ExpenseCategorySummary
): CategoryBodyGridCell => {
  return new CategoryBodyGridCell(
    summary.getCategory(),
    new MonthTotal(MonthTotal.FAKE_MONTH, summary.getAverage()),
    null
  );
};
...
export class ExpenseCategoryTableGridRow {
  ...
  constructor(id: number | string, cells: Array<DataTableRecord>) {
    this.id = id;
    this.cells = cells;
  }
...
export default class ExpenseCategoryTableGrid {
  ...
  constructor(
    ...
    this.grid = this.summaries.reduce(
      (
        grid: Array<ExpenseCategoryTableGridRow>,
        summary: ExpenseCategorySummary
      ): Array<ExpenseCategoryTableGridRow> => {
  ...
  getColumns(
    ...
    return this.grid.reduce(
      (
        result: Array<ExpenseCategoryTableGridRow>,
        row: ExpenseCategoryTableGridRow
      ): Array<ExpenseCategoryTableGridRow> => {
        return [
          ...result,
          new ExpenseCategoryTableGridRow(
            row.getId(),
            row.getCells(startingIndex, count)
          ),
        ];
      },
      []
  ...
  sort(columnIndex: number, direction: string): ExpenseCategoryTableGrid {
    ...
    return new ExpenseCategoryTableGrid(
      direction === 'desc' ? summaries.reverse() : summaries,
